...
def get_provider_prices_and_fees(url):
    ...
    for p in providers:
        ...
        if le:
            prijs = next((float(m.group().replace('.', '').replace(',', '.'))
                          for e in le.parent.next_elements if not isinstance(e, str)
                          for m in [re.search(r"\d{1,3}(?:\.\d{3})*(?:,\d{2})?", e.get_text(strip=True) if hasattr(e, 'get_text') else '')] if m), None)
            if prijs is not None: prijzen[p] = prijs
